...
def handle_user_deactivated(func: callable) -> callable:
    """
    Декоратор для обработки исключений при отправке сообщения пользователю,
    если ошибка UserDeactivated или BotBlocked(в библиотеке нет такой ошибки и телеграм не предоставляет
    возможности узнать заблокировал ли тея другой юзер), то меняем статус пользователя на dead.
